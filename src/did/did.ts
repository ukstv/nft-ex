...
export class DID {
    ...
    async verifyJWS(jws: string | DagJWS, options: VerifyJWSOptions = {}): Promise<VerifyJWSResult> {
        ...
        const header = base64urlToJSON(jws.split('.')[0]!)
        ...
        // const issuer = options.issuer || header['iss']
        const issuer = options.issuer
        if (issuer && issuer !== signerDid) {
            const issuerUrl = didWithTime(issuer, options.atTime)
